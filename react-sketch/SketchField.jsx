...
class SketchField extends PureComponent {
  ...
  componentDidMount = () => {
    ...
    let canvas = (this._fc = isPreview
      ? new fabric.StaticCanvas(this._canvas)
      : new fabric.Canvas(this._canvas, {
    ...
    canvas.on("selection:updated", (e) =>
      this.callEvent(e, this._onSelectionUpdated)
    );
    // IText Events fired on Adding Text
    // canvas.on("text:event:changed", console.log)
    // canvas.on("text:selection:changed", console.log)
    canvas.on("text:editing:entered", (e) => {
      if (e && e.target && e.target.hiddenTextarea)
        e.target.hiddenTextarea.setAttribute("id", "i-text-hidden");
    });
    canvas.on("text:editing:exited", (e) =>
      this.callEvent(e, this._onTextExited)
    );
    // window events
    fabric.util.addListener(window, "dblclick", (e) =>
      this.callEvent(e, this._onWindowDoubleClick)
    );
  ...
  componentWillUnmount = () => {
    ...
    window.removeEventListener("dblclick", (e) =>
      this.callEvent(e, this._onWindowDoubleClick)
    );
  };
